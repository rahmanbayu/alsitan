			...
			<div class="overflow-hidden bg-white p-5 px-4 shadow-sm sm:rounded-lg">
				<form action="{{ route('users.update', $user) }}" method="post">
					...
					<div>
						...
						<input value="{{ old('name', $user->name) }}" type="text" id="name" name="name" class="@error('name') border-rose-500  focus:border-rose-400 focus:ring-rose-200 active:border-rose-400 @else border-gray-300  focus:border-sky-400 focus:ring-sky-200 active:border-sky-400  @enderror w-full rounded border bg-white px-2 py-1.5 shadow outline-none ring-2 ring-transparent transition-all duration-150 active:shadow-sm">
						@error('name')
					...
					<div class="mt-6">
						...
						<input value="{{ old('email', $user->email) }}" type="email" id="email" name="email" class="@error('email') border-rose-500  focus:border-rose-400 focus:ring-rose-200 active:border-rose-400 @else border-gray-300  focus:border-sky-400 focus:ring-sky-200 active:border-sky-400  @enderror w-full rounded border bg-white px-2 py-1.5 shadow outline-none ring-2 ring-transparent transition-all duration-150 active:shadow-sm">
						@error('email')
							<div class="mt-1 text-sm text-rose-500">{{ $message }}</div>
						@enderror
					</div>
					<div class="mt-6">
						...
						<select name="role" id="role" class="@error('role') border-rose-500  focus:border-rose-400 focus:ring-rose-200 active:border-rose-400 @else border-gray-300  focus:border-sky-400 focus:ring-sky-200 active:border-sky-400  @enderror w-full rounded border bg-white px-2 py-1.5 shadow outline-none ring-2 ring-transparent transition-all duration-150 active:shadow-sm">
							...
							<option {{ old('role', $user->role) == 'staf' ? 'selected' : '' }} value="staf">staf</option>
							<option {{ old('role', $user->role) == 'kadis' ? 'selected' : '' }} value="kadis">kadis</option>
						</select>
						@error('role')
							<div class="mt-1 text-sm text-rose-500">{{ $message }}</div>
						@enderror
					...
					<div class="mt-6">
						<label for="password" class="mb-1.5 block text-sm font-bold">Password<sup class="text-xs text-gray-500">(optional)</sup></label>
					...
					<div class="mt-6">
						<label for="password_confirmation" class="mb-1.5 block text-sm font-bold">Konfirmasi Password<sup class="text-xs text-gray-500">(optional)</sup></label>
