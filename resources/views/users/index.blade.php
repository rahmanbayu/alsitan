<x-app-layout>
	<x-slot name="header">
		...
		<a href="{{ route('users.create') }}" class="flex items-center justify-center space-x-1 rounded bg-sky-500 px-3 py-2 text-xs uppercase text-white">
								...
								<table class="min-w-full text-left text-sm font-light">
									...
									<tbody class='text-sm text-gray-600 dark:text-gray-400'>
										@forelse ($users as $key => $item)
											<tr>
												<td class="whitespace-nowrap px-2 py-4">{{ $users->firstItem() + $key }}</td>
												<td class="max-w-[12rem] truncate whitespace-nowrap px-2 py-4">{{ $item->name }}</td>
												<td class="whitespace-nowrap px-2 py-4">{{ $item->email }}</td>
												<td class="whitespace-nowrap px-2 py-4">{{ $item->role }}</td>
												<td class="whitespace-nowrap px-2 py-4 text-right">
													<a href="{{ route('users.edit', $item) }}" class="inline-flex items-center space-x-1 rounded-sm bg-emerald-400 px-2 py-0.5 text-[0.6rem] uppercase text-white shadow-md">
